...
##   godot --headless --path . -s tests/test_anim_map.gd
...
##    preaviso salían a 12×, un temblor) es que hay que mirar la DURACIÓN, no el nombre.
	...
	for legend in LegendData.ABILITIES:
		...
		for i in abilities.size():
			...
			for key in ["anim", "anim_charged"]:
				...
				var dur: float = maxf(float(ab.get("adur", 0.0)), maxf(windup, 0.35))
				var speed: float = float(_lens[name]) / dur
				_check(speed >= MIN_SPEED and speed <= MAX_SPEED,
					"%s: %s dura %.2f s y sale a %.1f× (fuera de %.1f-%.1f; ajusta \"adur\")" % [
						what, name, float(_lens[name]), speed, MIN_SPEED, MAX_SPEED])
